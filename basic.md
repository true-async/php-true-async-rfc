...
| **Context Inheritance**   | Each `Scope` has a `.context`; child scopes inherit from parents. There's also a coroutine-level local context. `TaskGroup` by default has no new context.                     | `ScopedValues` or `ThreadLocal` for carrying data in virtual threads. Not directly integrated with `StructuredTaskScope`.   |
